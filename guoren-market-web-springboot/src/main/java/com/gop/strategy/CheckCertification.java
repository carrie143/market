...
@Service("checkCertification")
public class CheckCertification implements AuthStrategy {
	@Autowired
	...
	public void pre(AuthContext authContext) {
		...
		Boolean result = user.getAuthLevel().isBigerOrEqual(AuthLevel.LEVEL1);
		
		if (!result) {
			throw new AppException(UserCodeConst.NO_CERTIFICATION_LEVEL1);
		}
	}
